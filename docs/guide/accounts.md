...
VoxVer Launcher 支持多种登录方式，满足不同玩家的需求。本章将详细介绍每种登录方式的使用方法、多账户管理以及安全存储机制。
...
VoxVer Launcher 采用微软官方推荐的 **Device Code Flow** 进行身份验证。这种方式无需在启动器中输入密码，安全性更高。

### 登录步骤

1. 进入 **账户** 页面，点击 **「微软登录」** 按钮
2. VoxVer Launcher 会自动打开系统默认浏览器，跳转到微软设备登录页面
...
8. 回到 VoxVer Launcher，启动器会自动完成 token 交换并保存登录凭证
9. 登录成功后，你的玩家头像、用户名和 UUID 会显示在账户列表中

### Device Code Flow 工作原理

```
VoxVer Launcher 启动器                    微软服务器                   浏览器
    |                             |                          |
    |-- 请求设备码 --------------->|                          |
    |<-- 返回设备码 + 用户码 -----|                          |
    |                             |                          |
...
VoxVer Launcher 支持同时添加多个账户，方便在不同身份之间快速切换。
...
如果你不再需要某个账户，可以将其从 VoxVer Launcher 中移除：
...
::: warning 注意
- 移除账户 **仅会清除** VoxVer Launcher 中保存的该账户的 token 和档案信息。
...
VoxVer Launcher 使用 Electron 的 **safeStorage** API 对账户 token 进行加密存储，确保你的登录凭证安全。
...
::: warning 安全提醒
- 请勿将 VoxVer Launcher 的配置文件分享给他人，其中包含你的加密 token。
- 如果你的微软账号密码已更改，VoxVer Launcher 中的 token 可能会失效，需要重新登录。
- 建议定期检查你的微软账户安全设置，开启两步验证。
:::

## 皮肤管理

VoxVer Launcher 内置 **皮肤管理** 功能，支持查看、缓存和渲染当前登录账户的 Minecraft 玩家皮肤。

### 皮肤缓存机制

1. 首次登录微软账户后，启动器会自动从 Mojang 服务器下载玩家皮肤
2. 皮肤文件会缓存在本地，下次启动时无需重新下载
3. 启动器内嵌 **皮肤渲染器**，可以实时预览 2D 正面 / 背面视图

### 查看皮肤

1. 进入 **账户** 页面
2. 在已登录的微软账户卡片上点击 **「查看皮肤」** 按钮
3. 弹窗会显示该玩家的：
   - 玩家名（Username）
   - UUID
   - 2D 皮肤正面视图
...
| 操作 | 说明 |
|------|------|
| 清除缓存 | 在皮肤弹窗中点击「清除缓存」，下次查看时会重新下载 |
| 强制刷新 | 当玩家在 [minecraft.net](https://minecraft.net) 更换皮肤后，可使用此功能立即拉取最新皮肤 |

::: tip 提示
- 皮肤缓存采用 SHA1 哈希命名，便于版本管理。
- 离线模式下的玩家使用默认的 Steve 皮肤，无法自定义。
- 皮肤管理功能仅适用于微软正版登录账户。
:::
