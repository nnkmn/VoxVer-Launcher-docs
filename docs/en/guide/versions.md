...
MCLa supports full Minecraft version management, including releases, snapshots, old versions, and April Fools versions.

## Version List

Go to the **Versions** page and click "Refresh Version List". MCLa will fetch the latest version list from the BMCLAPI mirror.
...
MCLa supports version isolation. Each version has its own independent `.minecraft` directory, so mods and configurations do not interfere with each other.
