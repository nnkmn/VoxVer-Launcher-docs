...
MCLa supports customizing the `.minecraft` directory path, which is useful for multi-launcher coexistence or disk space management.
...
4. Save settings
5. **Restart MCLa** for changes to take effect
...
- Set an independent game directory for MCLa
...
Import an existing `.minecraft` directory as an MCLa instance:
